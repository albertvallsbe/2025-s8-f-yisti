...
import { fetchUsers, deleteUser, createUser } from '../../features/users/usersSlice';
...
jest.mock('../../components/RowsList/RowsList', () => ({
	RowsList: ({ onEdit, onDelete }: { onEdit: Function; onDelete: Function }) => (
		<div data-testid="rows-list-mock">
			<button onClick={() => onEdit({ id: 1, name: 'Test User' })}>Edit User</button>
			<button onClick={() => onDelete(1)}>Delete User</button>
		</div>
	),
}));
jest.mock('../../components/Modals/UserFormModal', () => ({
	UserFormModal: ({ isOpen, onSubmit }: { isOpen: boolean; onSubmit: Function }) =>
		isOpen ? (
			<div data-testid="user-form-modal-mock">
				<button onClick={() => onSubmit({ name: 'New User', password: 'password123', role: 'user' })}>
					Submit Form
				</button>
			</div>
		) : null,
...
describe('UsersPage', () => {
	...
	it('should dispatch fetchUsers when status is "idle"', () => {
		...
		expect(mockDispatch).toHaveBeenCalledWith(mockedFetchUsers.mock.results[0].value);
	});

	it('should open the modal in create mode when "Afegir usuari" is clicked', async () => {
		render(<UsersPage />);
		await userEvent.click(screen.getByRole('button', { name: /afegir usuari/i }));
	...
	it('should dispatch createUser when submitting the form in create mode', async () => {
		...
		expect(mockDispatch).toHaveBeenCalledWith(mockedCreateUser.mock.results[0].value);
		expect(screen.queryByTestId('user-form-modal-mock')).not.toBeInTheDocument();
	});
	...
	it('should dispatch deleteUser when onDelete is called and confirmed', async () => {
		...
		expect(mockDispatch).toHaveBeenCalledWith(mockedDeleteUser.mock.results[0].value);
	});
